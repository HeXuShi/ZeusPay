    ...
    public class AlipaySignature
    {
        ...
        public static string RSASignCharSet(string data, string privateKeyPem, string charset, bool keyFromFile, string signType)
        {
            ...
            try
            {
                ...
                if (null == rsaCsp)
                {
                    throw new InvalidOperationException("您使用的私钥格式错误，请检查RSA私钥配置" + ",charset = " + charset);
                }
                if ("RSA2".Equals(signType))
                {
                                             ...
                                             bool isDecrypt)
        {
            ...
            if (isCheckSign)
            {
                ...
                {
                    throw new InvalidOperationException("rsaCheck failure:rsaParams=" + parameters);
                }
            }
        ...
        public static string RSAEncrypt(string content, string publicKeyPem, string charset)
        {
            ...
            {
                throw new InvalidOperationException("EncryptContent = " + content + ",charset = " + charset, ex);
            }
        }
        ...
        public static string RSADecrypt(string content, string privateKeyPem, string charset, string signType)
        {
            ...
            {
                throw new InvalidOperationException("DecryptContent = " + content + ",charset = " + charset, ex);
            }
        }
